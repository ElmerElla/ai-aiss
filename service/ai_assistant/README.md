...
- **容器编排**：当前 Docker Compose 仅用于启动 Redis 缓存；MySQL 使用本地数据库实例，后端服务通过本地 `uvicorn` 手动启动。
...
### 基础部署（Redis 容器 + 本地 MySQL + 本地 uvicorn）
...
3. **配置环境**:
   - cp .env.example .env
   - 修改 .env 文件，确保 `MYSQL_HOST=127.0.0.1`、`REDIS_HOST=127.0.0.1`，并填入本地 MySQL 账号密码、API_KEY 等。
4. **一键启动**:
   `ash
   docker-compose up -d redis
   `
   *说明：此命令仅拉起 Redis 缓存容器。MySQL 由你本机自行维护，后端服务通过 `uvicorn` 手动启动。*
...
以下步骤用于本地开发场景（推荐）：先启用 Python 虚拟环境，再启动 FastAPI 后端。

### 5.1 进入后端目录

```bash
cd service/ai_assistant
```

### 5.2 创建并启用虚拟环境

1. 创建虚拟环境（目录名可自定义为 `.venv`）：
...
然后按实际环境修改 `.env`（MySQL、Redis、DashScope、百炼检索等配置）。
...
### 5.7 Docker 方式（可选，仅 Redis）

如果你希望用 Docker 启动 Redis，可使用：
...
更多接口详情请参考 [FastAPI 自动文档](http://localhost:8000/docs)。