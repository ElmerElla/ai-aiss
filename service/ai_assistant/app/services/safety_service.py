...
功能介绍：
-----------
本模块负责检测用户输入中的危险内容（自杀/自残/暴力倾向）和隐私违规行为。

检测机制：
1. 公共服务联系方式查询放行（如校医院电话、心理健康中心）
2. LLM 感性判断：使用 qwen-turbo 结合语境判断真实危险意图
3. 正则兜底：LLM 不可用或格式异常时，回退到关键词正则匹配

隐私检测：
- 识别查询文本中是否包含他人学号（5 位以上数字）
- 阻止学生查询其他同学的信息

输出：
- is_dangerous(): 返回布尔值，是否包含危险内容
- is_privacy_violation(): 返回被查询的学号或 None
"""
